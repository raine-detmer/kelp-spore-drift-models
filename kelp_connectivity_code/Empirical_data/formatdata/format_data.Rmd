...
#working in utm so need to account for grid divergence: true north is not 0º in utm
...
# longitude of central meridian for utm zone 11 is 117ºW
...
rect_and_rot <- function(x,xsize,ysize, head){
  ...
  poly2 <- rotate.polygon(poly, angle = head + 90, sp = FALSE, anchor = "center")%>% st_set_crs(32611)#use rotate.polygon() to rotate the polygon, not sure why I need to add 90º but I do. Then set the coordinate system to make sure the units are correct
...
# santa barbara: 34.4208° N, 119.6982° W; los angeles: 34.0522° N, 118.2437° W
...
The swell height data were collected by the Coastal Data Information Program (CDIP;  http://cdip.ucsd.edu/MOP_v1.1/) and formatted to give the quarterly maximum wave height in each Landsat pixel in the southern California Bight by Bell et al. in prep. The original data were provided in netcdf format; data from pixels in the study region were subsetted out and converted to csv's in Matlab (see LandsatWaves.m)
...
waves1d <- read.csv("../../../intermediate_data_output/raw_waves/waves1.csv", header = F) # max swell height in each pixel in each quarter of each year (first half)
waves2d <- read.csv("../../../intermediate_data_output/raw_waves/waves2.csv", header = F) # max swell height in each pixel in each quarter of each year (second half)

wavesd <- rbind(waves1d, waves2d) # join the wave data back together
